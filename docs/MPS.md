...
## LoRA は ISP のピークメモリを削減しない（2026-09-17 実測、各条件 3 回）

**観点**: LoRA（PEFT）を使うと、ISP のピークメモリが下がるか。

**基準**: 削減効果があると言うには、**LoRA あり**のピークが**LoRA なし**を下回ること（差 < 0）。
同一の入力・遺伝子・細胞数・バッチ・系列長で比較し、**各条件 3 回**反復します。
...
| forward バッチ | LoRA なし | LoRA あり | 差 | 判定 |
|---|---|---|---|---|
| 4 | **8.74 GiB**（8.74 / 8.74 / 8.74） | **8.84 GiB**（8.84 / 8.84 / 8.84） | **+0.10 GiB（+1.1%）**（3 回とも同値） | 削減なし |
| 2 | **5.04 GiB**（5.04 / 5.04 / 5.04） | **5.04 GiB**（5.04 / 5.04 / 5.04） | **±0.00 GiB**（3 回とも同値） | 削減なし |
...
- 推論では base の重みを常に保持する必要があり、アダプタとその演算の分だけ**増えます**。
  batch 4 の **+0.10 GiB は 3 回とも同値**で、ばらつきではなく LoRA 経路に固有の増加です。
  増加量がアダプタ本体（約 4 MiB）より大きいのは、LoRA が**行列積を 2 段（A → B）に増やす**ためです。
- batch 2 では差が 0.00 GiB でした。**ピークが小さい条件では、増加分がアロケータの粒度に収まります。**
- ピークを決めるのは **`バッチ × ヘッド数 × 系列長²`** の attention 活性です。この項は LoRA では
  変わりません（batch 4 で 8.74 GiB、batch 2 で 5.04 GiB と、ほぼ半減することはこの式と整合します）。
- **メモリを下げたい場合は `--batch` を下げます。** LoRA の有無は手段になりません。

**計測上の注意（2026-09-17 の訂正）**

初回は 1 条件 1 回で報告し、テンソル側のピークも併記しました（batch 4 で 6.10 / 7.16 GiB）。
反復するとテンソル側は 5.28〜5.64 GiB と**実行ごとに変動**したため、**ドライバのピークを主指標**とし、
テンソル側は参考値に格下げしました。ドライバのピークは 4 回の実行で 8.74 / 8.84 GiB と安定しています。
**主指標と参考値を分けずに引用しないでください。**

**この結果の使いどころ**: LoRA の利点は学習（メモリと計算）にあり、推論（ISP）にはありません。
ISP のメモリ見積もりでは、LoRA の有無を条件に入れる必要はありません（入れる場合は +1% 程度の増加として扱います）。
